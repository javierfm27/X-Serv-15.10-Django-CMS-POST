...
def logeado(request):
    if request.user.is_authenticated():
        headerHtml = "Logged in as " + request.user.username \
...
@csrf_exempt
def editar(request, recurso):
    logged, bodyHtml = logeado(request)
    if logged:
        try:
            pagina = Pages.objects.get(nombreRec=recurso)
            formulario = "<p><form id='editar' method='POST'>" \
                + "<label> Introduce la modificacion que quieras añadir " \
                + "</br></label>" \
                + "<textarea name='page' rows='20' cols='100' ></textarea>" \
                + "</br>" \
                + "<input type='submit' value='Enviar'></form></p>"
            if request.method == 'GET':
                return HttpResponse(bodyHtml + pagina.contenido + formulario)
            elif request.method == 'POST':
                modificacion = request.POST['page']
                return HttpResponse(bodyHtml + pagina.contenido + modificacion + formulario)
            else:
                return HttpResponseBadRequest("NO ES POSIBLE PARA LA OPERACION " + request.method)
        except Pages.DoesNotExist:
            return HttpResponseNotFound("No existe pagina para " + recurso)
    else:
        return HttpResponse(bodyHtml)
